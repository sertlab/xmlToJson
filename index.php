...
$xml = '<priip>
 ...
 function xmlDecode($myxml,$array = []) {


        foreach ($myxml as $key => $value) {


            if(is_object($value)){
                $array[] = [$key => xmlDecode((array)$value)];
            }else {
 ...
 function jsonEncode($array,$endarray=false,$arraycount=0,$json=null)
{
    foreach ($array as $key => $value) {
        if (is_array($value)){
           
            if(!is_numeric($key)){
                $json .='"'. $key.'"' . ":" ."[" . jsonEncode($value,$endarray,count($value));   
            }else{
                if ($key == $arraycount-1){
                    $json .= '{'.jsonEncode($value,$endarray = true);    
                }else {
                    $json .= '{'.jsonEncode($value,$endarray = false);
                }
